...
#define BUFFER_SIZE 10000 // taille du buffer circulaire
...
// 4 paramètre pour les protocoles : slow start, congestion avoidance, fast retransmit, fast recovery, 
// alpha, window_return, window_init, timer, window_factor, buffer_size, rto, rto_factor,
...
int debug_mode;
float alpha; // représente le poids qu'on donne à l'historique 
...
float beta; // pour le calcul du rttvar 
int rttvar;
//voir : Improved RTO Algorithm for TCP Retransmission Timeout

double timer; // le nombre de microsecondes durant lesquels le serveur va attendre un ACK quand sa fenêtre est vide
...
void reset_var(float alpha_val, int window_size_val,int window_factor_val, int window_return_val, int debug_mode_val, float beta_val, int rto_factor_val){ // on va reset les variables pour les nouveaux clients 
    cwnd = window_size_val; // mettre ça dans une fonction, pas en local 
    window_size = cwnd;
    window_factor = window_factor_val;
    window_return = window_return_val; 
    timer_set = 0;
    max_ack = 0;
    flightsize = 0;
    ssthresh = 2147483647;
...
// paramètres TCP
    ...
    retransmission = 0;
    alpha = alpha_val; // représente le poids qu'on donne à l'historique 
    
    timer = 15000; // le nombre de microsecondes durant lesquels le serveur va attendre un ACK quand sa fenêtre est vide
    rto_enable = 0; 
    beta = beta_val;
    rto = 10000; 
    rto_factor = rto_factor_val;
    debug_mode = debug_mode_val;
    // printf("window_size = %d\n",cwnd);
    // printf("window_factor = %d\n",window_factor);
    // printf("window_return = %d\n",window_return);
    // printf("alpha = %f\n",alpha);
    // printf("beta = %f\n",beta);
    // printf("rto_factor = %d\n",rto_factor);
}
...
double maj_timer(struct segment segments[], int seq){
    ...
    //     exit(0);
    // }
    int rtt = calcul_rtt(segments,seq);
    timer = alpha*timer+(1-alpha)*rtt;
    if(seq==1){
        rttvar = rtt/2;
    }
    if((int)timer<0){
        ...
        printf("Trans : %f secondes et  %f millisecondes  \n",(double)segments[(seq-1)%buff_size].trans_time.tv_sec,(double)segments[(seq-1)%buff_size].trans_time.tv_usec);
        printf("Last Recv : %f secondes et  %f millisecondes  \n",(double)segments[(seq-1)%buff_size].last_ACK_recv_time.tv_sec,(double)segments[(seq-1)%buff_size].last_ACK_recv_time.tv_usec);
        printf("Calcul rtt : %d et timer = %d\n",(int)calcul_rtt(segments,seq),(int)timer);
        exit(0);
    }
    if(rto_enable){
        rttvar = rttvar*(1-beta)+beta*(abs(timer-rtt)); 
        //rto = timer + rto_factor*rttvar;
        rto = 20000;
    }
}
...
void handle_DACK(struct segment segments[]){
    ...
    if(fast_recovery){
        ...
        if(debug_mode){
            printf("DACK : SLOW START => CONGESTION AVOIDANCE\n");
        }
        
        //cwnd = WINDOW/segments[(nv_ack-1)%buff_size].nbr_ACK_recv; 
        cwnd = window_return;    
...
void handle_ACK(struct segment segments[]){
    ...
    if(window_size>ssthresh && congestion_avoidance){ // CONGESTION AVOIDANCE => cwnd += 1/cwnd pour chaque ACK
        ...
        cwnd += window_factor*(nv_ack-max_ack);
    }              
    max_ack = nv_ack;
    // si pas dans l'ordre, donc inférieur au plus grand reçu => pas de pb 
    // si duplicate alors => retransmission 
    if(segments[(nv_ack-1)%buff_size].nbr_ACK_recv >= 5){ // si on a reçu 3 fois ou plus le même ACK
        lost_seg ++;
    ...
    if(strcmp(message,"ACK")==0){
        ...
        if(nv_ack>=max_ack && nv_ack!= 0){
            ...
            if(timer_set = 1){ // si notre fenêtre était à 0, alors on considère que le serveur n'a pas reçu d'ACK  
                ...
                if(segments[(nv_ack-1)%buff_size].nbr_ACK_recv >= 3){
                    cwnd = 1;
                }
                window_size =  (cwnd == 0) ? 1 : cwnd;
            }
...
void wait_for_ACK(int data_socket, struct sockaddr_in data_addr, int len, struct segment segments[], FILE *fp){
    ...
    if(window_size==0 || seq==(int)nbre_seg+1){ // si la fenêtre est à 0 alors on attend un ACK avec un timer de valeur RTT 
        ...
        tv.tv_usec = timer; // ... millisecondes .., on met 2*le RTT car il y a de la latence
        //tv.tv_usec = 0;
        if(timer<0){
            printf("error, timer négatif \n");
            exit(0);
...
int main(int argc, char **argv) {
    ...
    if(argc >= 9){
        ...
        window_factor = atoi(argv[4]); 
        window_return = atoi(argv[5]); 
        beta = atof(argv[6]); 
        rto_factor = atoi(argv[7]); 
    ...
    while(1){
        reset_var(alpha,window_size, window_factor, window_return, debug_mode, beta, rto_factor);
