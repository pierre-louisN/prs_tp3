...
#define PORT2	 1234
#define MAXLINE 1024
#define WINDOW 120 // taille initiale de la fenêtre (quand on a une perte on va revenir à cette valeur)
...
void reset_var(float alpha_val, int window_size_val,int window_factor_val, int window_return_val, int debug_mode_val){ // on va reset les variables pour les nouveaux clients 
...
// paramètres TCP
    ...
    printf("alpha = %f\n",alpha);
    timer = 10000; // le nombre de microsecondes durant lesquels le serveur va attendre un ACK quand sa fenêtre est vide
    rto_enable = 0; 
    rto = 2*timer; 
...
void handle_DACK(struct segment segments[]){
    ...
    }else if(!fast_retransmit && !fast_recovery){	
        if(debug_mode){
...
int twh_serv(int sockfd, struct sockaddr_in cliaddr){
    ...
    if((strcmp(buffer,"SYN")) == 0){ //si le client s'est bien connecté alors on crée la socket de données
        ...
        int port2 = PORT2;
        createsocket(&data_socket,&data_addr,(port2+nbre_clients));
        char message[12] = "SYN-ACK";
        char port[5];
        sprintf(port, "%d", (port2+nbre_clients));
        strcat(message,port);
        // message[12] = '\0';
...
int main(int argc, char **argv) {
    ...
    while(1){
        reset_var(alpha,window_size, window_factor, window_return, debug_mode);
